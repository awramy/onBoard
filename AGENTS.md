...
- **Session flow**: `POST /sessions` (create, status=planned) → `POST /sessions/:id/start` (generates questions, status=in_progress) → `GET /sessions/:id/current-question` → `POST /sessions/:id/answer` (AI evaluates, updates progress, advances) or `POST /sessions/:id/skip` (score=0, advances). Session auto-completes when last question is answered/skipped. Manual completion: `POST /sessions/:id/finish` (computes score, updates user league) or `POST /sessions/:id/abandon` (status=abandoned, progress preserved).
- **Finish endpoint**: `POST /sessions/:id/finish` — only for `in_progress` sessions. Computes avgScore from all InterviewAnswers, adds to `User.fullScore`, recalculates league (bronze < 100, silver < 500, gold < 1000, platinum >= 1000). Returns `{ sessionId, status, totalQuestions, questionsAnswered, avgScore, sessionScore, newFullScore, newLeague }`.
- **Abandon endpoint**: `POST /sessions/:id/abandon` — only for `in_progress` sessions. Sets status to `'abandoned'`, finishedAt = now(). Progress on answered questions is preserved. Returns `{ sessionId, status }`.
- **Answer endpoint**: `POST /sessions/:id/answer` with `{ answerText: string }`. Uses `AiService.evaluateAnswer()` with full context (question, explanation, isDivide, previous answers, mastery). Returns `{ answerId, score, feedback, isFullyClosed, recommendations, currentOrder, status, isFinished, nextQuestion? }`. Model selected from `session.config.model` (default `"auto"`). On `isFullyClosed`, mastery is set to 1.0.
- **ProgressModule**: Reusable service at `backend/src/progress/` for reading/writing `UserQuestionProgress` and `UserTopicProgress`. Used by SessionsModule (skip, answer). Exported and importable by any module.
